...
# HDD-режим (2026-09-23, прямой запрос "проработай алгоритм для hdd") —
# крупнее, чем DEFAULT_PACK_SIZE (используется для SSD): на вращающемся
# диске каждый новый файл — это операции с метаданными ФС (создание
# inode/записи в каталоге), которые сами по себе требуют seek, отдельно
# от содержимого. Меньше файлов = меньше таких пауз на гигабайт данных,
# при той же строго последовательной записи внутри каждого файла (см.
# depot_sync_manager.py::DepotSyncManager для выбора между HDD/SSD
# режимом и обоснование параллелизма заливки).
HDD_PACK_SIZE = 1024 * 1024 * 1024      # 1GB — для сервера на HDD
